...
#define HARD_VERSION "03"
#define SOFT_VERSION "v2.1.29"
#define DEVICE_TYPE_E "IJLY_410"
#define DEVICE_SN "000001"
#define SOFT_VERSION_SHELL "cat /etc/sysinfo.conf |grep soft_version|cut -c 14-|tr -s [\"\n\"]"
...
#define SHttpReport "jsonstr={\"VER\":\"%s\",\"CTEI\":\"%s%s\",\"MAC\":\"%s\",\"IP\":\"%s\",\"UPLINKMAC\":\"%s\",\"LINK\":\"%s\",\"FWVER\":\"%s\", \"DATE\":\"%s\"}"
...
int sendCheckSmartDev(char *anfangid,char *sendmsgData)
{
    ...
    if(startDev->dev == NULL )
    {
        ...
        while(PCur)
        {
          ...
          {
            sprintf(buf,"{\"id\":\"%s\",\"type\":\"1\",\"new\":\"1\"}",PCur->data.id);
...
int read_ver()
{
  ...
  if(fp== NULL) 
  {
    ...
    deviceVer[strlen(deviceVer)-1]=0;  
  }
  pclose(fp);
  return 1;  
}
...
int getUciValue(struct uci_context *c, char *key, char *value)
{
  char buf[64];
  struct uci_ptr p;
  sprintf(buf, "httppost.server.%s", key);
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
    sprintf(value, "");
  }
...
void initReportConfig(struct uci_context *c)
{
  ...
  sprintf(buf, "httppost.server.devicetype");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  sprintf(buf, "httppost.server.reportaddr");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  sprintf(buf, "httppost.server.reportport");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  sprintf(buf, "httppost.server.postaddr");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  sprintf(buf, "httppost.server.postport");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  sprintf(buf, "httppost.server.postpath");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
  ...
  else
  {
    ...
    sprintf(buf, "httppost.server.devicesn");
  if (UCI_OK != uci_lookup_ptr(c, &p, buf, true))
  {
    sprintf(deviceSn, DEVICE_SN);
  }
  else
  {
    if (p.o != NULL)
    {
      sprintf(deviceSn, p.o->v.string);
    }
    else
    {
      sprintf(deviceSn, DEVICE_SN);
...
int main(int argc, char *argv[])
{
  ...
  while (1)
  {
   ...
   //   printf("tz report port %s %s\n", reportServerIp, reportServerPort);
    ...
    dNow = localtime(&tNow);
    sprintf(bufNow,"%d-%d-%d %d:%d:%d", dNow->tm_year + 1900, dNow->tm_mon + 1, dNow->tm_mday,dNow->tm_hour,dNow->tm_min,dNow->tm_sec);
    ...
    if (dns_ok == 1)
    {
      if (homeOrRoute % 2 == 0)
      {
        ...
        sprintf(sendData, SHttpReport, HARD_VERSION,"180001300",deviceSn, deviceMacFu,wanIpaddr,deviceMacFu,"2",deviceVer,bufNow);
        if (debug_mode > 0)
          printf("send ok\n%s\n", sendData);
        httppost(sendData, strlen(sendData));
      }
    }
    /* 从服务器接收数据，并写入文件 */
